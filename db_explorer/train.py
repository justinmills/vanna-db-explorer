...
def train_from_db(vn: VannaBase) -> None:
    ...
    # The information schema query may need some tweaking depending on your
    # database. This is a good starting point.
    logger.info("Running sql to list public schema columns")
    df_information_schema = vn.run_sql(
        "SELECT * FROM information_schema.columns WHERE table_schema = 'public'"
    )
    # Now generate a plan from the above schema. This will break up the
    # information schema into bite-sized chunks that can be referenced by the
    # LLM
    ...
    logger.info("Training with some sql statements sql statements")
    # TODO: add a question for each of these (question=..., sql=...)
    sqls = [
        "select * from country c join countrylanguage l on c.code = l.countrycode;",
        "select * from country where continent = 'Africa';",
        "select * from countrylanguage where isofficial = true;",
        "select * from countrylanguage where isofficial = false;",
    ]
    for sql in sqls:
        vn.train(sql=sql)
